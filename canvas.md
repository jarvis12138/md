...
http://bbs.blueidea.com/thread-3105514-1-1.html
...
<body>
	<div id="box"></div>
	<canvas id="myCanvas" width="600" height="400">cann't canvas</canvas>
	<script type="text/javascript">
		...
		img.onload = function(){
			...
			// console.log(Math.sqrt(imageData.data.length));
			...
			for(var i=0; i<img.width; i++){
				for(var j=0; j<img.height; j++){
					// str[i][j] = 0;
					// console.log(imageData.data[i+j]);
					var positionP = i*4*img.width+4*j;
					if(imageData.data[positionP] == 0 && imageData.data[positionP+1] ==0 && imageData.data[positionP+2] == 0){
						// str[i] = [];
						
					}else{
						str[i][j] = imageData.data[positionP];
					}
				}
			}
			// for(var i=0; i<imageData.data.length; i++){
			// 	if(imageData.data[i] > 0){
			// 		str.push(imageData.data[i]);
			// 	}
			// }
			console.log(str);
			document.getElementById('box').innerHTML = str;
			ctx.clearRect(0, 0, canvas.width, canvas.height);
			// for(var o=0; o<imageData.data.length; o++){
			// 	canvas.ctx.fillStyle = dfdf;
			// }
